...
def start_alarm(sound):
    ...
    playsound("C:/driver_alert_system_using_CNN/data/drowsiness alert alert.mp3")


classes = ['Closed', 'Open']
face_cascade = cv2.CascadeClassifier("C:/driver_alert_system_using_CNN/data/haarcascade_frontalface_default.xml")
left_eye_cascade = cv2.CascadeClassifier("C:/driver_alert_system_using_CNN/data/haarcascade_lefteye_2splits.xml")
right_eye_cascade = cv2.CascadeClassifier("C:/driver_alert_system_using_CNN/data/haarcascade_righteye_2splits.xml")
cap = cv2.VideoCapture(0)
model = load_model("C:/driver_alert_system_using_CNN/drowiness_detector.h5")
count = 0
alarm_on = False
alarm_sound = "C:/driver_alert_system_using_CNN/data/drowsiness alert alert.mp3"
status1 = ''
status2 = ''
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if status1 == 2 and status2 == 2:
            ...
            if count >= 1:
                cv2.putText(frame, "Drowsiness Detected!!!", (100, height-20), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                if not alarm_on:
                    alarm_on = True
